	...
	<div class="container">
		<div class="OrderTopFlex">
			...
			<div class="ax_200" v-if="activeIndex == 3 || activeIndex == 4 || activeIndex == 5 ">
				<el-date-picker v-model="ProductionPrice.payTime" type="datetimerange" range-separator="-" value-format="yyyy-MM-dd HH:mm:ss"
				 start-placeholder="订单付款开始日期" end-placeholder="订单付款结束日期">
				</el-date-picker>
			</div>
			<div class="ax_200" v-if="activeIndex == 5">
				<el-date-picker v-model="ProductionPrice.startTimeArrory" type="datetimerange" range-separator="-" value-format="yyyy-MM-dd HH:mm:ss"
				 start-placeholder="订单完成开始日期" end-placeholder="订单完成结束日期">
				</el-date-picker>
			</div>
			<div class="ax_200">
	...
	<div class="ax_200">
			...
			<div class="ax_200">
				<el-select v-model="ProductionPrice.type" clearable placeholder="请选择付款方式">
					<el-option v-for="item in paymethod" :key="item.value" :label="item.label" :value="item.value">
					</el-option>
				</el-select>
			</div>
		...
		<el-menu :default-active="activeIndex" class="el-menu-demo" mode="horizontal" @select="handleSelect" id="u9" style="margin-bottom: 20px;">
			<el-menu-item index="1">全部<span v-if="activeIndex == 1">({{thistotal}})</span></el-menu-item>
			<el-menu-item index="2">待付款<span v-if="activeIndex == 2">({{thistotal}})</span></el-menu-item>
			<el-menu-item index="3">待发货<span v-if="activeIndex == 3">({{thistotal}})</span></el-menu-item>
			<el-menu-item index="4">待收货<span v-if="activeIndex == 4">({{thistotal}})</span></el-menu-item>
			<el-menu-item index="5">交易成功<span v-if="activeIndex == 5">({{thistotal}})</span></el-menu-item>
			<el-menu-item index="6">交易关闭<span v-if="activeIndex == 6">({{thistotal}})</span></el-menu-item>
			<el-menu-item index="7">已取消<span v-if="activeIndex == 7">({{thistotal}})</span></el-menu-item>
		</el-menu>
		...
		<el-table v-loading="listLoading" :data="list" element-loading-text="Loading" max-height="650" border fit highlight-current-row>
			...
			<el-table-column label="交易完成时间" align="center">
				<template slot-scope="scope">
					<div v-if="3 === scope.row.orderStatus">
			...
			<el-table-column label="尾款" align="center">
				...
				<el-table-column label="店主名称" align="center">
			...
			<el-table-column label="订单状态" align="center">
				<template slot-scope="scope">
					...
					<div v-if="2 === scope.row.orderStatus">
						待收货
					</div>
					<div v-if="3 === scope.row.orderStatus">
						交易完成
					</div>
					<div v-if="4 === scope.row.orderStatus">
						交易失败
					</div>
					<div v-if="5 === scope.row.orderStatus">
						已取消
					</div>
				</template>
			</el-table-column>
			<el-table-column label="付款方式" align="center">
				<template slot-scope="scope">
					...
					<div v-if="2 === scope.row.type">
						微信
					</div>
					<div v-if="3 === scope.row.type">
						支付宝
					</div>
