<template>
  <div class="app-container">
    ...
    >
	  ...
	  <el-table-column label="绑定活动id" align="center">
	    <template slot-scope="scope">
      ...
      <el-table-column label="状态" align="center">
        ...
        <template slot-scope="{row}">
          <el-button v-if="row.status == 1" type="primary" size="mini" @click="activityEdit(row)">编辑</el-button>
           <el-button type="primary" size="mini" @click="activityForbidden(row)">{{ row.status == 1 ? '下架' : '上架' }}</el-button>
		   <el-button v-if="row.status == 0" type="info" size="mini" @click="activitydelete(row)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
...
<script>
import {activityGoodsList, activityDelApi, activityForbApi,activityGoodsStatusChange,activityGoodsDelete } from "@/api/activity.js";
...
export default {
  ...
  methods: {
    ...
    toAddActivity(){
      this.$router.push({ path: "activityGoodsAdd"});
    },
    activityDel(row){
	...
	activitydelete(row){
		  console.log(row,'row')
		   this.$confirm('此操作将删除该活动, 是否继续?', '提示', {
		      ...
		      activityGoodsDelete({ id:row.id }).then(res=>{
		        if( res.code == 0 ){
		          this.$message({
		            type: 'success',
		            message: '操作成功!'
    ...
    activityForbidden(row){
      console.log(row,'row')
       this.$confirm('此操作将上架/下架该活动, 是否继续?', '提示', {
          ...
          activityGoodsStatusChange({ id:row.id,status }).then(res=>{
            if( res.code == 0 ){
              this.$message({
                type: 'success',
                message: '操作成功!'
    ...
    activityEdit(row){
      this.$router.push({ path: "activityGoodsAdd",query: { id: row.id,edit:'edit'}} );
    },
  }
};
</script>
