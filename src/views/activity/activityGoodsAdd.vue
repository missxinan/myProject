		...
		<el-card class="box-card">
			...
			<el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
				...
				<el-form-item label="商品价格" prop="goodsPrice">
					<el-input type="textarea" v-model="ruleForm.goodsPrice"></el-input>
				</el-form-item>
				<el-form-item label="商品规则人数" prop="theRules">
					<el-input type="textarea" v-model="ruleForm.theRules"></el-input>
				</el-form-item>
				<!-- <el-form-item label="商品类型" prop="goodsType">
				  <el-radio-group v-model="ruleForm.goodsType">
				    <el-radio label="1" value="1">抽奖商品</el-radio>
				    <el-radio label="2" value="2">赠送商品</el-radio>
				  </el-radio-group>
				</el-form-item> -->
				<el-form-item label="封面图选择" prop="headImage" class="is-required" v-if="this.saveType != 'see'">
				...
				<el-form-item label="绑定活动">
					<el-select style="width:350px" v-model="ruleForm.activityId"  placeholder="请选择展示区域">
					  <el-option
...
<script>
	...
	export default {
		...
		methods: {
			...
			getGoodsDetail(){
				activityGoodsInfo({id:this.$route.query.id}).then(res=>{
					if(res.code == 0){
						res.data.headImage = res.data.headImage
			...
			editGoods(index){
				this.$refs["ruleForm"].validate(valid => {
					if (valid) {
						...
						obj.headImage = obj.headImage;
						obj.goodsType?"":obj.goodsType=2
						obj.status=1
						activityGoodsEdit(obj).then((res) => {
								if (res.code == 0) {
									this.$router.push({path:'/activity/activityGoods',query:{	upload: 'upload'}})
								} else {
			...
			addGoods(index) {
				this.$refs["ruleForm"].validate(valid => {
					if (valid) {
						...
						obj.status=1
						console.log(obj)
						activityGoodsNewAdd(obj).then((res) => {
								if (res.code == 0) {
									this.$router.push({path:'/activity/activityGoods',query:{	upload: 'upload'}})
								} else {
									this.$message.error(res.msg);
				...
				uploadFileList(formDate).then(resp => {
					...
					if (resp.code == 0) {
						...
						imgObj.onload = function(){
							that.ruleForm.headImage = img_url.split('?')[0] + '?' + imgObj.width + '*' + imgObj.height + '/' + img_url.split('?')[1]
						}
					} else {
						this.$message({
							message: resp.msg,
