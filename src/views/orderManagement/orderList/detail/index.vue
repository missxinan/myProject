<template>
  <div class="container">
    ...
    <el-card class="box-card" v-if="bigobj.orderStatus != 0 && bigobj.integralType == 2">
      <div slot="header" class="clearfix">
        <span>赠送明细</span>
      </div>
      <div>
        <span style="margin-right:30px;">积分：{{bigobj.integralRatio}}</span>
        <span>超值积分：{{bigobj.worthRatio}}</span>
      </div>
      <div>
        <div style="margin:20px 0;">赠送商品：</div>
        <el-table
          :data="giveGoodsList"
        ...
        >
          ...
          <el-table-column label="积分" align="center">
            <template slot-scope="scope">{{ scope.row.totalIntegral }}</template>
          </el-table-column>
          <el-table-column label="操作" align="center">
            <template slot-scope="scope">
              <span
                @click="seeToMallDetail(scope.row)"
                style="color: #0066cc;
  cursor: pointer;"
              >查看详情</span>
            </template>
...
export default {
  ...
  created() {
    ...
    let parentIndexType = this.$route.query.parentIndexType;
    // console.log(parentIndexType,'aaaa')
    // console.log(this.$route.query,'bbbb')
  },
  destroyed() {
  ...
  methods: {
    ...
    getOrderDetail() {
      ...
      if (Data[0] == "orderId") {
        ...
        orderdetaillist({
          ...
          // console.log(this.bigobj.integralType,'我是类型')
          ...
          this.giveGoodsList = res.data.giveOrderList;
    ...
    seeToMallDetail(row) {
      ...
      this.$router.push({
        ...
        query: { orderId:row.orderId,parentIndexType:this.parentIndexType}
      });
    }
  }
};
