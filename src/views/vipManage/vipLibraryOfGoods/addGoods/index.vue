		...
		<el-card class="box-card">
			...
			<el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
				...
				<el-form-item label="商品名称" prop="goodsName" >
					<el-input  v-model="ruleForm.goodsName" maxlength="50" ></el-input>
				</el-form-item>
				<el-form-item label="商品简介" prop="description">
					<el-input type="textarea" v-model="ruleForm.description" maxlength="50"></el-input>
				</el-form-item>
				...
				<el-form-item label="成本价" prop="costPrice">
					<el-input v-model="ruleForm.costPrice" maxlength="12" oninput="value=value.replace(/[^\d.]/g,'')"></el-input>
				</el-form-item>
				<!-- <el-form-item label="商品限价" v-if="saveType == ''" prop="limitedPriceType"> -->
				...
				<el-form-item v-if="ruleForm.limitedPriceType == 1 " label="限价价格" maxlength="10"  :prop=" ruleForm.isLimitPrice == 1 ? 'limitedPrice' : ''">
					<el-input v-model="ruleForm.limitedPrice" oninput="value=value.replace(/[^\d.]/g,'')"></el-input>
				</el-form-item>
				<el-form-item label="起拍价" prop="startPrice"  >
					<el-input v-model="ruleForm.startPrice" maxlength="12" oninput="value=value.replace(/[^\d.]/g,'')"></el-input>
				</el-form-item>
				<el-form-item label="加价幅度" prop="markupPrice" >
					<el-input v-model.number="ruleForm.markupPrice" maxlength="7"></el-input>
				</el-form-item>
			<!-- 	<el-form-item label="库存" prop="inventory">
					<el-input v-model.number="ruleForm.inventory" maxlength="7" :disabled=" saveType == '' ? false : true"></el-input>
				</el-form-item> -->
        <el-form-item label="赠送积分比" prop="isGiveIntegral">
        	<el-input type="number" v-model="ruleForm.isGiveIntegral" oninput="value=value.replace(/[^\d.]/g,'')" maxlength="4" ></el-input>
				...
				<el-form-item label="估值" prop="assessPrice">
					<el-input v-model="ruleForm.assessPrice" maxlength="12" oninput="value=value.replace(/[^\d.]/g,'')"></el-input>
				</el-form-item>
			</el-form>
		</el-card>
		<el-card class="box-card">
...
<script>
	...
	import {vipLibraryOfGoodsAdd, vipLibraryOfGoodsEdit, vipLibraryOfGoodsDetails} from "@/api/vipMange.js";
	import {
		treeApi
		...
		data() {
			return {
				...
				rules: {
					...
					markupPrice: [{
						required: true,
						message: '请输入加价幅度（必须为整数）',
						trigger: 'blur'},
						{ type: 'number', message: '加价幅度必须为整数'}
					],
					isGiveIntegral: [{
						required: true,
						message: '请输商品积分赠送比例（必须在0-1之间）',
						trigger: 'blur'},
					],
					limitedPrice:[{
						required: true,
		...
		methods: {
			...
			cancel() {
				if(this.$route.query.type && this.$route.query.type == "2"){
							this.$router.push({path:'/vipMange/vipLibraryOfGoods/vipCommissionAuction'})
						} else {
							this.$router.push({path:'/vipMange/vipLibraryOfGoods/vipLibraryList'})
						}
			},
			//键盘输入事件
			submitadd() {
			...
			saveshopping() {
				...
				if (Obj.headImage.length > 0 && Obj.logo.length > 0) {
					...
					if (this.copyflag) {
						vipLibraryOfGoodsAdd(Obj).then((res) => {
							if (res.msg === 'success') {
			...
			getGoodsDetail(){
				...
				vipLibraryOfGoodsDetails({goodsId:this.$route.query.goodsId}).then(res=>{
					if(res.code == 0){
			...
			editGoods(index){
				...
				obj.headImage = JSON.stringify(obj.headImage);
				obj.status = index
				vipLibraryOfGoodsEdit(obj).then((res) => {
						if (res.code == 0) {
							if(this.$route.query.type && this.$route.query.type == "2"){
							this.$router.push({path:'/vipMange/vipLibraryOfGoods/vipCommissionAuction'})
						} else {
			...
			addGoods(index) {
				...
				vipLibraryOfGoodsAdd(obj).then((res) => {
						if (res.code == 0) {
						if(this.$route.query.type && this.$route.query.type == "2"){
							this.$router.push({path:'/vipMange/vipLibraryOfGoods/vipCommissionAuction',query:{	upload: 'upload'}})
						} else {
							this.$router.push({path:'/vipMange/vipLibraryOfGoods/vipLibraryList',query:{	upload: 'upload'}})
						}
						} else {
							this.$message(res.msg);
						}
