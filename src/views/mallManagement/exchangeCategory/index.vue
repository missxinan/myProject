<template>
	<div class="table-box">
		...
		<tree-table :data="data" v-on:handleCreate="handleCreate" v-on:handleUpdate="handleUpdate" v-on:handleDelete="handleDelete" max-height="700"
		...
		<el-dialog :visible.sync="dialogPvVisible" title="Reading statistics">
			<el-table :data="pvData" border fit highlight-current-row style="width: 100%" max-height="600">
		...
		methods: {
			...
			recursionfunc(obj) {
				...
				for (let k in obj) {
					...
					//
					if (k === 'categoryId') {
						newObj['categoryName'] = obj[k].length === 4 ? '一级' : obj[k].length === 8 ?'二级': '三级';
					}
			...
			handleCreate(row) {
				if (row.menuId === 0) {
					...
					}else if(row.categoryId.length === 12){
						this.$message({
							message: '最多只能添加到三级',
							type: 'warning'
						});
