<template>
	<div class="mallOrderDetail">
			...
			<div class="info-content">
				<div class="order-status">
					...
					<div class="order-status">
						...
						<div v-if="orderDetail.orderStatus === 1">
							{{orderDetail.orderType === 0? '待发货':orderDetail.orderType === 1?'待接拍':orderDetail.orderType === 2?'寄拍中':'寄拍审核不通过'}}
						</div>
						<div v-if="orderDetail.orderStatus === 2">待收货</div>
						<div v-if="orderDetail.orderStatus === 3">
							{{orderDetail.orderType === 3? '寄拍成功': '交易完成'}}
						</div>
				...
				<div class="order-time">
					...
					<div v-if="orderDetail.orderStatus === 2">
						...
						<div class="item-times">快递公司（单号）：{{orderDetail.logisticsName}} ({{orderDetail.logisticsNo}})</div>
					</div>
					<div v-if="orderDetail.orderStatus === 3">
		...
		<div class="user-Info" v-if="(orderDetail.orderStatus === 1 && orderDetail.orderType === 1 ) || (orderDetail.orderStatus === 1 && orderDetail.orderType === 2 ) || (orderDetail.orderStatus === 3 && orderDetail.orderType === 3 )">
			<div class="info-title">
				<span class="title-line"></span>
		...
		<div class="order-btn">
			<el-button class="item-btn" v-if="orderDetail.orderStatus === 1 && orderDetail.orderType === 1 && checkPermission(['mall:mallOrder:acceptGoods'])" type="primary" @click="receiveGoods">接拍</el-button>
			<el-button class="item-btn" v-if="orderDetail.orderStatus === 1 || orderDetail.orderStatus === 2 && checkPermission(['mall:mallOrder:changeRemark'])" type="primary" @click="editRemark">修改备注</el-button>
			<el-button class="item-btn" v-if="orderDetail.orderType === 1 || orderDetail.orderType === 2 || orderDetail.orderType === 3" type="primary" @click="seecontract">查看合同</el-button>
			<el-button class="item-btn" v-if="orderDetail.orderStatus === 1 && (orderDetail.orderType === 0 || orderDetail.orderType === 4 || orderDetail.orderType === 5) && checkPermission(['mall:mallOrder:sendGoods'])" type="primary" @click="deliver">发货</el-button>
		</div>
		<el-dialog
			title="发货"
...
<script>
	...
	import { orderDetailMall,acceptAuctionMall,deliverGoodsMall,editRemarkMall } from "@/api/mallManagement"
			...
			getMallOrder(){
				...
				orderDetailMall(datas).then((res) =>{
					if(res.code == 0){
						...
						that.goodsDetail.push(res.data.goodsInfo);
					}else{
						that.$message({
							message: res.msg,
							type: 'error'
