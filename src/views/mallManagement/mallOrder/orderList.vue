  ...
  <div class="orderList">
    <div class="orderList-maill">
      <el-tabs v-model="parentIndex" type="card" @tab-click="handleClick">
		...
		<el-tab-pane name="4" label="直购"></el-tab-pane>
      </el-tabs>
    </div>
...
export default {
  ...
  methods: {
    ...
    exportToExcel() {
      ...
      // 	return
      // }
      // if( (new Date(this.createTime[1]).getTime() - new Date(this.createTime[0]).getTime()) > 30*24*60*60*1000 ){
      // 	this.$message.error('只能导一个月内的数据');
      // 	return
      //   }
      let times = 0;
      let url = "upms/mallOrder/exportNotComplete";
      if (this.createTime) {
        if (
          ...
          times += 1;
          url = "upms/mallOrder/exportNotComplete";
        }
      }
      if (this.payTime) {
        if (
      ...
      if (this.CompleteTime) {
        if (
          ...
          times += 1;
          url = "upms/mallOrder/exportMallingComplete";
        }
      }
      if (this.ReceivingGoodsTime) {
        if (
          ...
          times += 1;
          url =
            "upms/mallOrder/exportExchangeComplete";
        }
      }
      if (times == 0) {
    ...
    lookOrder(row) {
      let that = this;
      that.$router.push({
        path: "/mallManagement/mallOrderDetail",
        query: {
          orderId: row.orderId,
