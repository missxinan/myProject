<template>
  <div class="app-container">
        ...
        >
		  ...
		  <el-table-column label="天数编号" align="center">
		    <template slot-scope="scope">
		      <span>{{ scope.row.dayNo }}</span>
		    </template>
		  </el-table-column>
		  <el-table-column label="芒果币系数" align="center">
		    <template slot-scope="scope">
		      <span>{{ scope.row.signScale }}</span>
		    </template>
		  </el-table-column>
		  <el-table-column label="签到赠送奖励数量" align="center">
		    <template slot-scope="scope">
		      <span>{{ scope.row.rewardNumber }}</span>
		    </template>
		  </el-table-column>
		  <el-table-column label="奖励图片" align="center" width="160">
		    <template slot-scope="scope">
		          <img class="goods_image" :src="scope.row.rewardImage" />
		    </template>
		  </el-table-column>
		  <el-table-column label="补签图片" align="center" width="160">
		    <template slot-scope="scope">
		          <img class="goods_image" :src="scope.row.suppleImage" />
		    </template>
		  </el-table-column>
      ...
      />
	  <!-- 编辑每日签到奖励 -->
	  <el-dialog title="编辑每日签到奖励" :visible.sync="dialogAuditVisible">
	    <el-form
	      ref="dataForm"
	      :model="temp"
	      label-position="left"
	    ...
	    >
		  <el-form-item label="芒果币系数:"> 
		    <el-input @input="change($event)" v-model="temp.signScale"/>
		  </el-form-item>
		  <el-form-item label="奖励数量:">
		    <el-input @input="change($event)" v-model="temp.rewardNumber"/>
		  </el-form-item>
		  <el-form-item label="签到图片">
		  	...
		  	<el-dialog :visible.sync="dialogVisible">
		  		<img width="100%" :src="temp.rewardImage" alt="">
		  	</el-dialog>
		  </el-form-item>
		  <el-form-item label="补签图片">
		  	<el-upload action="" list-type="picture-card" :limit="1" :auto-upload="false"
		  	 :on-change="getFile1" :on-preview="handlePictureCardPreviewHeader1" :on-remove="handleRemove1">
		  		<i class="el-icon-plus"></i>
		  	</el-upload>
		  	<el-dialog :visible.sync="dialogVisible1">
		  		<img width="100%" :src="temp.suppleImage" alt="">
		  	</el-dialog>
		  </el-form-item>
	    </el-form>
  ...
  methods: {
    ...
    getList() {
      this.listLoading = true;
        fetchSignList(this.userlistQuery).then(response => {
	...
	addOrUpdataAudit(){
	  this.$refs["dataForm"].validate(valid => {
	    if (valid) {
			...
			fetchSignEdit(this.temp).then(res => {
		...
		uploadFileList(formDate).then(resp => {
			...
			if (resp.code == 0) {
				...
				imgObj.onload = function(){
						that.temp.rewardImage = img_url.split('?')[0] + '?' + imgObj.width + '*' + imgObj.height + '/' + img_url.split('?')[1]	
						that.$forceUpdate()
				}
			} else {
				this.$message({
	...
	getFile1(file, fileList) {
		...
		uploadFileList(formDate).then(resp => {
			...
			if (resp.code == 0) {
				...
				imgObj.src = img_url;//改变图片地址
				imgObj.onload = function(){
						that.temp.suppleImage = img_url.split('?')[0] + '?' + imgObj.width + '*' + imgObj.height + '/' + img_url.split('?')[1]	
						that.$forceUpdate()
				}
			} else {
				this.$message({
