<template>
  <div class="app-container">
		...
		<div v-if="parentIndex=='2'" v-loading="loading">
			...
			<div class="btn-type">
				...
				<el-button
				  class="filter-item"
				  type="primary"
				  @click="handleAreaClass"
				>修改区域类别</el-button>
				<p style="width: 20px;"></p>
				<el-select
				  v-model="operateStatus"
				  clearable
				  class="filter-item"
				  style="width: 130px"
				  placeholder="区域代理状态"
				...
				<el-button
				  class="filter-item"
				  type="primary"
				  @click="handleAreaStatus"
				>修改区域状态</el-button>
				<p style="width: 20px;"></p>
				<el-select
				  v-model="operateHidden"
				  clearable
				  class="filter-item"
				  style="width: 130px"
				  placeholder="区域隐藏状态"
				...
				<el-button
				  class="filter-item"
				  type="primary"
				  @click="handleAreahidden"
				>修改显示状态</el-button>
			</div>
			<p class="desc">*红色：未代理， 灰色：保护区， 蓝色：已合作，橘黄：已交定金，黑色：区域隐藏</p>
			<div class="cityDetail">
				<div class="flexDiv"  v-for="(item) in indexCountyInfo" :key="item.areaCode">
					...
					<div class="cityli">
						<span :style="{background:group.hidden==1?'#000000':''}"
						:class="group.signStatus==1?'danger0':group.signStatus==2?'danger3':group.signStatus==3?'danger2':group.signStatus==4?'danger1':''"
						 v-for="(group) in  item.countyList" 
						 :key="group.areaCode">{{group.areaName}} {{group.level}} <el-checkbox @change="selectChange(group)" v-model="group.checked"></el-checkbox></span>
					</div>
				</div>	  
			</div>
		</div>
		...
		<div class="padd" v-if="parentIndex=='3'" v-loading="loading">
			...
			<div class="classify">
				<p>*红色：未代理， 灰色：保护区， 蓝色：已合作，橘黄：已交定金，黑色：区域隐藏</p>
				<div class="agentDetail">
					<span :style="{background:item.hidden==1?'#000000':''}"
					:class="item.signStatus==1?'danger0':item.signStatus==2?'danger3':item.signStatus==3?'danger2':item.signStatus==4?'danger1':''"
					 ...
					 :key="item.areaCode">{{item.areaName}} {{item.level}} <el-checkbox @change="selectChange(item)" v-model="item.checked"></el-checkbox></span>
				</div>
			</div>
		</div>
...
export default {
  ...
  methods: {
	...
	handleAreaClass(){
		console.log('修改区域类别')
		if(this.changeAreaList.length==0){
			this.$message("请选择修改区域！");
			return
		}
		if(!this.operateClass){
			this.$message("请选择修改类别！");
			return
		}
		let changeList=""
		this.changeAreaList?changeList=this.changeAreaList.join(","):""
		console.log(this.changeAreaList)
		let data={
			areaCodes:changeList,
			level:this.operateClass
		}
		updateAreaClass(data).then(response => {
		  console.log(response)
		  this.getCountyList()
		  this.changeAreaList=[]
	...
	handleAreahidden(){
		...
		let data={
			areaCodes:changeList,
			hidden:this.operateHidden?this.operateHidden:0
		}
		updateHidden(data).then(response => {
		  console.log(response)
		  this.getCountyList()
		  this.changeAreaList=[]
		  this.$message(response.data);
	...
	selectChange(item){
		console.log(item)
		if(item.checked){
			this.changeAreaList.push(item.areaCode)
		}else{
			console.log(this.changeAreaList.indexOf(item.areaCode))
			if(this.changeAreaList.indexOf(item.areaCode)!=-1){
				this.changeAreaList.splice(this.changeAreaList.indexOf(item.areaCode),1)
			}
		}
	},
	getAllCity(){
		...
		fetchCountyInfo(data).then(response => {
		  ...
		  if (response.code == 0) {
		    ...
		    for(let i=0;i<arrData.length;i++){
		    			  for(let j=0;j<arrData[i].countyList.length;j++){
		    				  arrData[i].countyList[j].checked=false
		    			  }
		    }
		    this.loading=false
...
.cityDetail .flexDiv .cityli{
	padding: 0 0 0 1.375rem;
}
.cityDetail .flexDiv .cityli>span{
	padding:0 1.5rem;
	height: 1.875rem;
	font-size: 1.25rem;
	white-space: nowrap;
...
.cityDetail .flexDiv .cityli>span.danger0{
	background: #ff5757;
	color: #fff;
	border: none;
}
.cityDetail .flexDiv .cityli>span.danger1{
	background: #f8f8f8;
	color: #c8c8c8;
	border: none;
}
.cityDetail .flexDiv .cityli>span.danger2{
	background: #57a2ff;
	color: #fff;
	border: none;
}
.cityDetail .flexDiv .cityli>span.danger3{
	background: #ffb200;
	color: #fff;
	border: none;
}
