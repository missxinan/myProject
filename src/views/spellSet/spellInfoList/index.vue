  ...
  <div class="container">
    <div class="addFlex">
	  ...
	  <el-select
	    ...
	    style="width: 200px"
	  >
	    <el-option label="查全部带手机号查下级" value="1"/>
	    <el-option label="查去除公司人员及下级 " value="2"/>
   ...
   <div class="ax_200 right">
    ...
    >
      <el-table-column label="统计日期" align="center" width="260">
        <template slot-scope="scope">{{ scope.row.dayTime }}</template>
      </el-table-column>
	  <el-table-column align="center" label="拼赚金额">
...
export default {
  ...
  methods: {
    ...
    getList() {
	  ...
	  this.listQuery.type =this.listQuery.type? this.listQuery.type : "3";
      ...
      spellUserInfo(this.listQuery).then(res => {
        if (res.code == 0) {
	...
	exportToExcel() {
	  let that = this;
	  let url = "/spell/make/order/userInfoExcel";
	  this.listQuery.type =this.listQuery.type? this.listQuery.type : "3";
	  this.listQuery.phone == "" ? (this.listQuery.phone = undefined) : "",
	  this.listQuery.time
	    ? ((this.listQuery.minDate = this.listQuery.time[0].substr(0,10)),
	      (this.listQuery.maxDate = this.listQuery.time[1].substr(0,10)))
	    : ((this.listQuery.minDate = undefined),
	      (this.listQuery.maxDate = undefined));
